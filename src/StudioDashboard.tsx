  ...
  return (
    ...
    >
      ...
      <div className="w-1/2 p-8 border-r border-neutral-700 overflow-auto relative">
        <button
          onClick={onClose}
          className="absolute left-4 top-4 text-neutral-500 hover:text-neutral-200 z-10 flex items-center gap-2"
        ...
        <button
          onClick={addPost}
          className="absolute top-4 right-4 flex h-10 w-10 items-center justify-center rounded-full bg-neutral-50/10 backdrop-blur-lg shadow-md transition active:scale-90 z-10"
          aria-label="New Post"
        >
          <div className="text-white">
            <Plus size={18} />
...
function PublicSitePreview({
  ...
  type View =
    ...
    | { type: "post"; project: Project; post: Post };
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b px-6 py-4 text-sm font-medium">
        <button
          onClick={back}
          className="flex items-center gap-1 text-neutral-500 hover:text-neutral-800 bg-white"
        >
            ...
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              {view.project.posts.map((post) => (
                <div
                  key={post.id}
                  onClick={() =>
                    setView({ type: "post", project: view.project, post })
                  }
              ...
              {view.post.body ? (
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: parseMarkdown(view.post.body),
                  }}
                />
              ) : (
...
export default function StudioDashboard() {
  ...
  const addProject = () => {
    ...
    const { x, y } = polar(
      160,
      (360 / (projects.length + 1)) * projects.length,
    );
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center text-neutral-50 selection:bg-neutral-700">
      <div
        ref={containerRef}
        className="relative h-[28rem] w-[28rem] select-none"
      >
      ...
      <RadialMenu open={menuOpen} actions={actions as any} />
      <AnimatePresence>
        {openProj && (
          <motion.div
      ...
      <p className="fixed bottom-8 left-8 text-xs text-neutral-500">
        ⌘P – New Project
      </p>
    </div>
  );
